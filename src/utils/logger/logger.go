...
func init() {
	for _, ma := range major {
		for _, mi := range minor {
			vers = append(vers, fmt.Sprintf("/root/.gvm/pkgsets/go1.%s.%s/global/src/github.com/rjrnd", ma, mi))
			vers = append(vers, fmt.Sprintf("%s/src/github.com/rjrnd", os.Getenv("GOPATH")))
		}
	}
}
...
func C(format string, args ...interface{}) { criticaldeps(3, format, args...) }
...
// Crtl pritns as critical
func Crtl(format string, args ...interface{}) {
	criticaldeps(3, format, args...)
}

func criticaldeps(deps int, format string, args ...interface{}) {
	s := fmt.Sprintf(format, args...)
	criticalLogger.Output(deps, s)

	if isSentry {
		_, fn, line, _ := runtime.Caller(deps - 1)
		s = fmt.Sprintf("[CRITICAL] %s:%d: %s", fn, line, s)
		raven.CaptureMessage(s, nil, raven.NewException(&CRITICAL{s}, trace(deps)))
	}
}
